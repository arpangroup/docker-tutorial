...
//                }])
        ...
        $nearMe = $nearMe->map(function ($restaurant) {
            ...
            return $restaurant->only(['id', 'name', 'description', 'latitude', 'longitude', 'image', 'rating', 'delivery_time', 'price_range', 'slug', 'is_featured', 'is_active', 'schedule', 'delivery_type']);
        });
    ...
    public function getSelfPickupRestaurants(Request $request)
    {
        ...
        $nearMe = $nearMe->map(function ($restaurant) {
            return $restaurant->only(['id', 'name', 'description', 'image', 'rating', 'delivery_time', 'price_range', 'slug', 'is_featured', 'is_active', 'distance', 'schedule', 'delivery_type']);
        });
    ...
    {
        Log::info('getRestaurantInfoAndOperationalStatus.....');
        ...
        if($request->payment_method){
         ...
         /* Step4: Check restaurant is active or not */
         if($restaurant->is_active ==0){
            $operationalStatus['is_operational'] = false;
            $operationalStatus['message'] = 'store is not active now';
         }


        /* Step5: Check restaurant is open or not */
        $scheduleData = $this->getScheduleData($restaurant);
        if($scheduleData['is_open'] == '0'){
            $operationalStatus['is_operational'] = false;
            $operationalStatus['message'] = $scheduleData['status'];
        }
        ...
        if($operationalStatus['is_operational'] == true){
            ...
            if($request->latitude && $request->longitude){
                if (config('settings.enDistanceMatrixDeliveryTime') == 'true' && ($restaurant->delivery_type == 1 || $restaurant->delivery_type == 3)) {
                    $distanceMatrixResponse = $this->getGoogleDistance($request->latitude, $request->longitude, $restaurant);
                    Log::info('GOOGLE_DISTANCE_MATRIX_RESPONSE: ..........' .json_encode($distanceMatrixResponse));
                    $distance = ($distanceMatrixResponse['distance']['value']) /1000;

                    $deliveryTimeInSecond = $distanceMatrixResponse['duration']['value'];
                    $deliveryTimeInMin = ($deliveryTimeInSecond /60) + ((int)$restaurant->delivery_time);
                    $restaurant['delivery_time'] = ceil($deliveryTimeInMin);
                    $restaurant['delivery_distance'] =  ceil($distance);
                ...
                // Calcute thedelivery charge
                Log::info('Calcute delivery charge for distance: ' . $distance);
                $deliveryCharge = 0;
                if($request->delivery_type == 1) {// if "DELIVERY"

                    if($restaurant->delivery_charge_type == 'DYNAMIC'){
                        Log::info('DYNAMIC deliveryCharge enabled');                           
                        
                        if ($distance > $restaurant->base_delivery_distance) {
                            Log::info('Distance is more than baseDistance ');
                            ...
                            if (config('settings.enDelChrRnd') == 'true') {
                                Log::info('DeliveryCharge RoundUp enabled ');
                                $dynamicDeliveryCharge = ceil($dynamicDeliveryCharge);    
                                Log::info('Server DeliveryCharge: ' .$dynamicDeliveryCharge);
                            }
    
                            $deliveryCharge = $dynamicDeliveryCharge;
                        } else {
                            Log::info('Applying base delivery charge');
                            $deliveryCharge = $restaurant->base_delivery_charge;
                        }
                    }else {
                        Log::info("FIXED deliveryCharge enabled");
                        $deliveryCharge = $restaurant->delivery_charges;
                    }
                }else{
                    Log::info('SelfPickup Order, so deliveryCharge: 0');
                    $deliveryCharge = 0;
                }
    ...
    { 
        ...
        if($request->latitude && $request->longitude){                    
            if (config('settings.enDistanceMatrixDeliveryTime') == 'true' && ($restaurant->delivery_type == 1 || $restaurant->delivery_type == 3)) {
                ...
                $deliveryTimeInMin = ($deliveryTimeInSecond /60) + ((int)$restaurant->delivery_time);
                $restaurant['delivery_time'] = ceil($deliveryTimeInMin);  
                $restaurant['delivery_distance'] =  $distance; 
                // Check is_operational or not:
                if ($distance <= $restaurant->delivery_radius){
