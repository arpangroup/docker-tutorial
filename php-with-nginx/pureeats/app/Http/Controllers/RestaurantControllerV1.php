...
class RestaurantControllerV1 extends Controller
{
    ...
    public function getRestaurants(Request $request){
        ...
        try{
            ...
            if($lat != null && !is_numeric($lat) )throw new ValidationException(ErrorCode::BAD_REQUEST, "invalid query parameter lat, it should be a valid numeric value");
            if($lng != null && !is_numeric($lng) )throw new ValidationException(ErrorCode::BAD_REQUEST, "invalid query parameter lng, it should be a valid numeric value");

            $restaurants = $this->storeService->getAllStores($lat, $lng);

            return response()->json([
                'success' => true,
                'count' => sizeof($restaurants),
                'message' => sizeof($restaurants) > 0 ? 'Total ' .sizeof($restaurants) .' restaurants found' : 'No restaurant found',
                'data' => $restaurants
            ]);
        }catch (\Throwable $th) {
            throw new ValidationException(ErrorCode::BAD_REQUEST, $th->getMessage());
        }

    }

    //http://192.168.0.100:8000/api/restaurant?id=1&slug=&lat=21.9726545&lng=87.3725539
    public function getRestaurantInfo(Request $request){
        ...
        try{
            if($id == null && $slug == null) throw new ValidationException(ErrorCode::BAD_REQUEST, "insufficient query parameter");
            if($lat != null && !is_numeric($lat) )throw new ValidationException(ErrorCode::BAD_REQUEST, "invalid query parameter lat, it should be a valid numeric value");
            if($lng != null && !is_numeric($lng) )throw new ValidationException(ErrorCode::BAD_REQUEST, "invalid query parameter lng, it should be a valid numeric value");
            if(isset($request['id']) && !is_numeric($id))throw new ValidationException(ErrorCode::BAD_REQUEST, "invalid query parameter id");
            if(isset($request['id']) && $id < 1)throw new ValidationException(ErrorCode::BAD_REQUEST, "invalid query parameter id");
            //if(!isset($request['id']) && isset($request['slug']) && $slug == null)throw new ValidationException(ErrorCode::BAD_REQUEST, "insufficient query parameter");

            //$restaurantInfo = $this->getRestaurantDetails($id, $slug, $lat, $lng);

            if($slug != null){
                $restaurantInfo = $this->storeService->getStoreBySlug($slug, $lat, $lng);
            }else{
                $restaurantInfo = $this->storeService->getStoreById($id, $lat, $lng);
            }
        ...
        if (Cache::has('store-info-' . $slugStr)) {
            ...
            if (config('settings.enDistanceMatrixDeliveryTime') == 'true') {
                ...
                foreach ($activeRestaurants as $restaurant) {
                    ...
                    if ($check) {
                        // Calculate the delivery time based on geographical distance
                        $distanceInKm = $this->getDistancgetRestaurantInfoe($lat, $lng, $restaurant->latitude, $restaurant->longitude);
                        $actualDeliveryTime = $distanceInKm * ((int)config('settings.approxDeliveryTimePerKm')) ;
                        $restaurantDeliveryTime = $actualDeliveryTime + ((int)$restaurant->delivery_time);
