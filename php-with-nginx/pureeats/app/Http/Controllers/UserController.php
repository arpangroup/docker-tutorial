        ...
        if($request->phone && $request->otp){
            ...
            if($verifyResponse['valid_otp'] == true){
                ...
                if($user){// user is available
                    ...
                    if($user->hasRole('Customer')) {//user has valid role
                        if($user->is_active == 1){
                            Log::info('Saving push token......');
                            if($request->push_token){
                                $this->savePushToken($user->id, $request->push_token);
                            ...
                            return response()->json([
                                'success' => true,
                                'message' => "user is authenticated for login",
                        ...
                        }else{// user is NOT_ACTIVE
                            throw new AuthenticationException(ErrorCode::ACCOUNT_BLOCKED, "User blocked");
                        }
                    }else{//user does not have valid role
                        Log::error('user does not have a valid customer role');
                        throw new AuthenticationException(ErrorCode::BAD_REQUEST, "Invalid Role");
                    }
                }else{// although OTP is verified, but user is not available in the system. so need registration
                    Log::info('although OTP is verified, but user is not available in the system. so registration required');
                    return response()->json([
                        'success' => true,
                        'message' => "verification success, user need to registration",
    ...
    */
    ...
    public function sendOTP(Request $request){
        ...
        if($request->phone != null){
            ...
            throw new AuthenticationException(ErrorCode::INVALID_AUTH_PROVIDER, "Invalid authentication provider");
        }
    }
    ...
    public function verifyPhone(Request $request){
        Log::info('#############################################################');
        //Log::info('Inside verifyPhone() ####IP_ADDRESS::' . $request->ip());
        //Log::info( $request->header('User-Agent'));
        //Log::info('verifyPhone: .', ['REQUEST' => $request->headers->all()]);
        Log::info($request->path());
        Log::info(json_encode($request->headers->all()));
        Log::info('#############################################################');
        if(strlen($request->phone) == 10){
            ...
            if($user){                
                ...
                return response()->json([
                    'success' => true,
                    'message' => "Verification successful for registration",
                    'status' => AuthStatus::CODE_SEND,
                    'code'=> '200',
                ]);
            }
    ...
    public function verifyEmail(Request $request){
        ...
        if($user){
            ...
            if($user->is_active){
                ...
                $email = new Email();
                $email->processEmailAction('OTP', $request->email);



                return response()->json([
                    'success' => true,
                    'message' => " OTP send successfully to " .$request->email,
                    'status' => AuthStatus::CODE_SEND,
                    'code'=> '200',
                ]);
            }else{
    ...
    public function updateUserInfo(Request $request)
    {
        ...
        if ($user) {            
            ...
            if($request->unique_order_id){
                $running_orders = \App\Order::where('user_id', $user->id)
                    ->whereIn('orderstatus_id', ['1', '2', '3', '4', '5', '6', '7', '8', '10', '11', '73', '710'])
