...
class CouponController extends Controller
{
    ...
    private function validateApplyCouponRequest($request){
        if($request->coupon && $request->restaurant_id && $request->subtotal && $request->payment_method){
            ...
            if(!$request->coupon)throw new ValidationException(ErrorCode::INVALID_REQUEST_BODY, "coupon should not be null");
            if(!$request->restaurant_id)throw new ValidationException(ErrorCode::INVALID_REQUEST_BODY, "restaurant_id should not be null");
            if(!$request->subtotal)throw new ValidationException(ErrorCode::INVALID_REQUEST_BODY, "subtotal should not be null");
            if(!$request->payment_method)throw new ValidationException(ErrorCode::INVALID_REQUEST_BODY, "payment_method should not be null");
        }
