...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        foreach ($orderStatusOrders as $key => $value) {
            ...
            if ($key == 10) {
                $orderStatusesData .= '{value:' . $value . ", name: 'Reached Pickup Location'},";
            }
            if ($key == 11) {
        ...
        try {
            ...
            if ($user->hasRole('Delivery Guy')) {
                ...
                if ($request->hasFile('delivery_photo')) {
                    ...
                    Image::make($photo)->resize(250, 250)->save(public_path('/assets/img/delivery/' . $filename));
 ...
 */
    public function saveSlide(Request $request)
    {
        ...
        Image::make($image)
            ->resize(384, 384)
            ->save(public_path('assets/img/slider/' . $filename));
 ...
 */
    public function saveNewRestaurant(Request $request)
    {
        ...
        Image::make($image)
            ->resize(160, 117)
            ->save(public_path('assets/img/restaurants/' . $filename), config('settings.uploadImageQuality '), 'jpg');
 ...
 */
    public function saveNewItem(Request $request)
    {
        ...
        Image::make($image)
            ->resize(486, 355)
            ->save(public_path('assets/img/items/' . $filename), config('settings.uploadImageQuality '), 'jpg');
 ...
 */
    ...
    {
        Log::channel('orderlog')->info('########## Inside cancelOrderFromAdmin()');
        ...
        try {
            if ($order->orderstatus_id != 5 || $order->orderstatus_id != 6) {
                ...
                if ($request->refund_type == 'NOREFUND' && $order->wallet_amount != null) {
                    Log::channel('orderlog')->info('########## NO_REFUND ' .$order->wallet_amount * 100);
                ...
                if ($request->refund_type == 'FULL') {
                    Log::channel('orderlog')->info('########## FULL_REFUND: ' .$order->total * 100);
                ...
                if ($request->refund_type == 'HALF') {
                    Log::channel('orderlog')->info('########## HALF_REFUND: ' .$order->total / 2 * 100);
                    $user->deposit($order->total / 2 * 100, ['description' => $translationData->orderPartialRefundWalletComment . $order->unique_order_id]);
                }

                //cancel order
                Log::channel('orderlog')->info('Saving order cancel status to database');
                $order->orderstatus_id = 6; //6 means canceled..
                $order->save();
                Log::channel('orderlog')->info('SAVED successfully');

                //throw notification to user
                if (config('settings.enablePushNotificationOrders') == 'true') {
                    $notify = new PushNotify();
                    Log::channel('orderlog')->info('Sending push notification to user: '.$order->user_id .' for order ID: ' .$order->unique_order_id);
 ...
 */
     ...
     * @param Request $request
     */
    public function markSelfPickupOrderAsCompletedFromAdmin(Request $request, TranslationHelper $translationHelper)
    {
        ...
        if ($order && $order->delivery_type =='2' && $order->orderstatus_id = '7') {        
